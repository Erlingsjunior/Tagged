...
export const useAuthStore = create<AuthState>((set, get) => ({
    ...
    register: async (email: string, password: string, nickname: string, name?: string, cpf?: string, phone?: string) => {
        try {
            ...
            // Get users database (apenas usuários reais, não mock)
            ...
            try {
                ...
                if (usersDbJson) {
                    ...
                    usersDb = Object.fromEntries(
                        Object.entries(parsed).filter(([email]) => !email.startsWith('user') || !email.includes('@tagged.com'))
                    );
                }
            } catch (e) {
            ...
            // Save to database (apenas usuários reais)
            usersDb[email] = {
                ...newUser,
    ...
    completeProfile: async (name: string, cpf: string, phone: string) => {
        try {
            ...
            // Get users database (apenas usuários reais)
            let usersDb: Record<string, any> = {};
            try {
                const usersDbJson = await AsyncStorage.getItem(STORAGE_KEYS.USERS_DB);
                if (usersDbJson) {
